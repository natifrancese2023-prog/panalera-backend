...
const anularFactura = async (id_factura, { motivo = null, id_usuario = null } = {}) => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    // 1. Bloqueo FOR UPDATE para evitar condiciones de carrera
    const resFactura = await client.query(
      `SELECT id_factura, estado FROM public.factura WHERE id_factura = $1 FOR UPDATE`,
      [id_factura]
    );

    if (resFactura.rowCount === 0) {
      throw new Error('La factura no existe');
    }

    const factura = resFactura.rows[0];

    // 2. Validar que no se encuentre previamente anulada
    if (factura.estado === 'anulada') {
      throw new Error('La factura ya se encuentra anulada');
    }

    // 3. Registrar la anulación contable
